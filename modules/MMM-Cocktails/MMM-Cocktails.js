...
Module.register("MMM-Cocktails", {
  ...
  start: function() {
    ...
    this.today = "";
    this.Cocktails = [];
    this.url = "http://www.thecocktaildb.com/api/json/v1/1/";
    this.scheduleUpdate();
  },
  ...
  getDom: function() {
    ...
    if (cocktails.strGlass === "vote" || " ") {
      ...
      title.innerHTML =
        cocktails.strDrink + "  ~  " + cocktails.strGlass + "<br>";
    }
  ...
  getCocktails: function(drink) {
    if (drink != undefined || typeof drink == "string") {
      this.sendSocketNotification(
        "GET_COCKTAILS",
        this.url + "search.php?s=" + encodeURI(drink)
      );
    } else {
      this.sendSocketNotification("GET_COCKTAILS", this.url + "random.php");
    }
  },
